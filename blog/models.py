...
class UserInfo(AbstractUser):#:可以使用auth模块验证
    ...
    class Meta:
        verbose_name = '用户'  # 给django-admin中表名变成中文
...
class Blog(models.Model):
    ...
    class Meta:
        verbose_name = '博客'  # 给django-admin中表名变成中文
        verbose_name_plural = verbose_name
...
class Category(models.Model):
    ...
    class Meta:
        verbose_name = '分类'  # 给django-admin中表名变成中文
        verbose_name_plural = verbose_name
...
class Tag(models.Model):
    ...
    class Meta:
        verbose_name = '标签'  # 给django-admin中表名变成中文
        verbose_name_plural = verbose_name
...
class Article(models.Model):
    ...
    class Meta:
        verbose_name = '文章'  # 给django-admin中表名变成中文
        verbose_name_plural = verbose_name
...
class ArticleDetail(models.Model):
    ...
    class Meta:
        verbose_name = '文章详情'  # 给django-admin中表名变成中文
        verbose_name_plural = verbose_name
...
class Article2Tag(models.Model):
    ...
    class Meta:
        ...
        verbose_name = '文章-标签'  # 给django-admin中表名变成中文
        verbose_name_plural = verbose_name
    def __str__(self):
...
class ArticleUpDown(models.Model):
    ...
    class Meta:
        # unique_together = (("article", "user"),)#联合唯一所以应
        unique_together = (
            ('article','user')
        )
        verbose_name = '点赞'  # 给django-admin中表名变成中文
        verbose_name_plural = verbose_name
...
class Comment(models.Model):
    ...
    create_time = models.DateTimeField(auto_now_add=True)
    parent_comment = models.ForeignKey("self", null=True, on_delete=models.CASCADE, blank=True)  # blank表示在django中该列可以不填

    def __str__(self):
        return self.content

    class Meta:
        verbose_name = '评论'  # 给django-admin中表名变成中文
        verbose_name_plural = verbose_name
